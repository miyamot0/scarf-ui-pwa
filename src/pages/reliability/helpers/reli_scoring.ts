import type { QuestionObjectHolder, StudyObject } from '@/questions/types/QuestionTypes';

export const CompareResponsesInAreas = (primary: QuestionObjectHolder[], reli: QuestionObjectHolder[]) => {
  let count = 0;
  let total = 0;

  primary.forEach((question) => {
    const primary_question = question;
    const reliability_question = reli.find((q) => q.QuestionID === primary_question.QuestionID);
...
export const CalculateAgreement = (primary: StudyObject[], reli: StudyObject[]): CalculateAgreementReturn => {
  let iv_count = 0;
  let iv_total = 0;
  let ev_count = 0;
  let ev_total = 0;
  ...
  primary.forEach((study) => {
    ...
    const reliability_study = reli.find((s) => s.StudyID === primary_study.StudyID);

    if (reliability_study) {
      const primary_internal = primary_study.InternalValidity.Questions;
      const reliability_internal = reliability_study.InternalValidity.Questions;

      const iv_number = CompareResponsesInAreas(primary_internal, reliability_internal);

      const primary_external = primary_study.ExternalValidity.Questions;
      const reliability_external = reliability_study.ExternalValidity.Questions;

      const ev_number = CompareResponsesInAreas(primary_external, reliability_external);
      ...
      const reporting_number = CompareResponsesInAreas(primary_reporting, reliability_reporting);

      const primary_outcomes = primary_study.Outcomes.Questions;
      const reliability_outcomes = reliability_study.Outcomes.Questions;

      const outcomes_number = CompareResponsesInAreas(primary_outcomes, reliability_outcomes);
  ...
  return {
    TotalCount: count,
    TotalTotal: total,
    TotalPercent: Math.floor((count / total) * 100),
    IV: {
      Count: iv_count,
      Total: iv_total,
      Percent: Math.floor((iv_count / iv_total) * 100),
    },
    EV: {
      Count: ev_count,
      Total: ev_total,
      Percent: Math.floor((ev_count / ev_total) * 100),
    },
    Reporting: {
      Count: reporting_count,
      Total: reporting_total,
      Percent: Math.floor((reporting_count / reporting_total) * 100),
    },
    Outcomes: {
      Count: outcomes_count,
      Total: outcomes_total,
      Percent: Math.floor((outcomes_count / outcomes_total) * 100),
    },
  };
};
